...
function calculateROI() {
  ...
  var savings_str = "$" + pensolve_savings.toFixed(2).replace(/./g, function(c, i, a) {
    return i && c !== "." && ((a.length - i) % 3 === 0) ? ',' + c : c;
});
  var cost_str = "$" + pensolve_savings.toFixed(2).replace(/./g, function(c, i, a) {
    return i && c !== "." && ((a.length - i) % 3 === 0) ? ',' + c : c;
});

  var divobj = document.getElementById('roi-answers');
  divobj.style.display = "block";
  divobj.innerHTML = "The annual cost of spreadsheet engineering: " + cost_str;
